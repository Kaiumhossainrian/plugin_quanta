...
class TP_Contact_Form extends Widget_Base {
    ...
    public function get_tp_contact_form(){
        ...
        $tp_cfa         = ['0' => esc_html__( 'Select Form', 'quanta' ) ];
        if( $tp_forms ){
            foreach ( $tp_forms as $tp_form ){
                $tp_cfa[$tp_form->ID] = $tp_form->post_title;
            }
        }else{
            $tp_cfa[ esc_html__( 'No contact form found', 'quanta' ) ] = 0;
        }
        return $tp_cfa;
    }
	...
	protected function register_controls() {
        ...
        $this->start_controls_section(
            'quanta_contact',
            [
                'label' => esc_html__('Contact Form', 'quanta'),
            ]
        );

        $this->add_control(
            'quanta_select_contact_form',
            [
                'label'   => esc_html__( 'Select Form', 'quanta' ),
                'type'    => Controls_Manager::SELECT,
                'default' => '0',
	...
	protected function render() {
        ...
        <div class="innerWrapperSidebar">
          <div class="sidebarWidget">
                <?php if( !empty($settings['quanta_select_contact_form']) ) : ?> 
                <div class="contact__form m-0"> 
                    <?php echo do_shortcode( '[contact-form-7  id="'.$settings['quanta_select_contact_form'].'"]' ); ?> 
                </div> 
                <?php else : ?>
                    <?php echo '<div class="alert alert-info"><p class="m-0">' . __('Please Select contact form.', 'quanta' ). '</p></div>'; ?>
                <?php endif; ?>            
          </div>
        </div>
